...
if DEBUG:
    ...
    def trace(*args):
        ...
        if DEBUG > 9:
            fargnames, _, _, flocals = inspect.getargvalues(
            ...
            print(
                "%s(%s): %s" % (fname, ", ".join(fargs), "; ".join(args),),
                file=sys.stderr,
            )
        elif DEBUG > 1:
            print("%s: %s" % (fname, "; ".join(args),), file=sys.stderr)
...
def fetch_url_urlopener(url):
    ...
    urlopener = UrlOpener()
    if ACCEPT_GZIP:
        urlopener.addheader("Accept-Encoding", "gzip,deflate")
    ...
    if DEBUG and rescode != 200:
        print(
            "fail: %d\n url: %s\ninfo: %s\nbody: %s\n"
            % (rescode, url, response.info(), response_content,),
            file=sys.stderr,
        )
    response.close()
...
def fetch_url_curl(url):
    try:
        ...
        rescode = 401
        if DEBUG:
            print("fail %d: %s" % (e.returncode, e.output), file=sys.stderr)
    return rescode, response_content
...
class YahooFinanceETL(object):
    ...
    def _scrape_data(self, url):
        ...
        if not self._cache.get(url):
            ...
            for i in range(0, max_retry):
                ...
                if rescode == 200:
                    ...
                    re_script = soup.find(
                        "script", text=re.compile("root.App.main")
                    )
                    ...
                    self._cache[url] = loads(
                        re.search("root.App.main\s+=\s+(\{.*\})", script).group(
                            1
                        )
                    )
                    ...
                    break
                print(
                    "Fail try %d, code %d from %s" % (i, rescode, url),
                    file=sys.stderr,
                )
                if i < max_retry - 1:
                ...
                print(
                    "Failed, code %d from %s" % (rescode, url), file=sys.stderr
                )
        ...
        try:
            ...
            print(
                "Failed, missing stores in %s bytes from url: %s"
                % (storekey, len(str(data)), url),
                file=sys.stderr,
            )
            raise
    ...
    def _clean_historical_data(self, hist_data, last_attempt=False):
        ...
        for k, v in hist_data.items():
            if k == "eventsData":
                ...
                if isinstance(v, list):
                    ...
                    for type_key, type_obj in v.items():
                        ...
                        for date_key, date_obj in type_obj.items():
                            ...
                            cleaned_date = self.format_date(
                                int(date_obj["date"])
                            )
                            date_obj.update({"formatted_date": cleaned_date})
                            formatted_type_obj.update(
                                {formatted_date_key: date_obj}
                            )
                ...
                sub_dict_list = []
                for sub_dict in v:
                    sub_dict["formatted_date"] = self.format_date(
                        sub_dict["date"]
                    )
    ...
    def _create_dict_ent(
        ...
        if statement_type == "history":
            ...
            try:
                cleaned_re_data = self._recursive_api_request(
                    hist_obj, up_ticker
                )
            except KeyError:
                try:
                    re_data = self._scrape_data(YAHOO_URL)[
    ...
    def get_stock_tech_data(self, tech_type):
        if tech_type == "defaultKeyStatistics":
            return self.get_stock_data(
                statement_type="keystats", tech_type=tech_type
            )
        else:
    ...
    def get_reformatted_stmt_data(self, raw_data, statement_type):
        ...
        for tick in self.ticker:
            sub_dict_ent = self._get_sub_dict_ent(
                tick, raw_data, statement_type
            )
...
class YahooFinancials(YahooFinanceETL):
    ...
    def _run_financial_stmt(self, statement_type, report_num, reformat):
        ...
        if reformat:
            raw_data = self.get_stock_data(
                statement_type, report_name=report_name
            )
    ...
    def get_financial_stmts(self, frequency, statement_type, reformat=True):
        ...
        if isinstance(statement_type, str):
            data = self._run_financial_stmt(
                statement_type, report_num, reformat
            )
        else:
            data = {}
            for stmt_type in statement_type:
                re_data = self._run_financial_stmt(
                    stmt_type, report_num, reformat
                )
                data.update(re_data)
        return data
    ...
    def get_stock_price_data(self, reformat=True):
        if reformat:
            return self.get_clean_data(
                self.get_stock_tech_data("price"), "price"
            )
        else:
    ...
    def get_key_statistics_data(self, reformat=True):
        if reformat:
            return self.get_clean_data(
                self.get_stock_tech_data("defaultKeyStatistics"),
    ...
    def get_stock_earnings_data(self, reformat=True):
        if reformat:
            return self.get_clean_data(
                self.get_stock_tech_data("earnings"), "earnings"
            )
        else:
    ...
    def _stock_price_data(self, data_field):
        ...
        for tick in self.ticker:
            if self.get_stock_price_data()[tick] is None:
                ...
                ret_obj.update(
                    {tick: self.get_stock_price_data()[tick].get(data_field)}
                )
        return ret_obj
    ...
    def _stock_summary_data(self, data_field):
        ...
        for tick in self.ticker:
            if self.get_summary_data()[tick] is None:
                ...
                ret_obj.update(
                    {tick: self.get_summary_data()[tick].get(data_field)}
                )
        return ret_obj
            ...
            if cur_market_cap[tick] is not None:
                if price_type == "current":
                    ...
                    if current[tick] is not None:
                        ret_obj.update(
                            {tick: cur_market_cap[tick] / current[tick]}
                        )
                    else:
                        ret_obj.update({tick: None})
                else:
                    if (
                        today_low[tick] is not None
                        and today_high[tick] is not None
                    ):
                        today_average = (today_high[tick] + today_low[tick]) / 2
                        ret_obj.update(
                            {tick: cur_market_cap[tick] / today_average}
                        )
                    else:
                        ret_obj.update({tick: None})
            else:
